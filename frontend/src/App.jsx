...
function App() {
  ...
  useEffect(() => {
    Promise.all([api('/api/rooms'), api('/api/equipment'), api('/api/venue')]).then(([roomData, equipmentData, venueData]) => {
      setRooms(roomData.rooms || []); setEquipment(equipmentData.equipment || []); setVenue(Object.fromEntries((venueData.content || []).map((item) => [item.key, item.value])))
    }).catch(() => {})
  }, [])
  ...
  return (
    <main className="shell">
      ...
      <section className="brand-panel">
        ...
        <div className="venue-name">{venue.name || 'BandRoom 音乐排练空间'}</div>
        <h1>让每一次排练，<span>准时开始。</span></h1>
        <p className="lead">{venue.description || '多房间预约、会员卡与排练设备，一处管理你的乐队时光。'}</p>
...
function OwnerManagement({ rooms, equipment, plans, onRefresh, onError }) {
  const [room, setRoom] = useState({ name: '', description: '', capacity: 6 }); const [item, setItem] = useState({ name: '', description: '', quantity: 1 }); const [plan, setPlan] = useState({ planType: 'monthly', name: '', priceCents: 0, includedUses: 10 }); const [card, setCard] = useState({ userId: 1, planId: plans[0]?.id || '', paidAmountCents: 0, paymentMethod: '线下支付', notes: '' }); const [schedule, setSchedule] = useState({ weekday: 1, opensAt: '08:00', closesAt: '23:00', enabled: true }); const [closure, setClosure] = useState({ startsAt: '', endsAt: '', reason: '' }); const [contentValues, setContentValues] = useState({}); const [photo, setPhoto] = useState({ roomId: '', url: '', sortOrder: 0 }); const [content, setContent] = useState({ key: 'description', value: '' }); const [message, setMessage] = useState('')
  useEffect(() => { api('/api/venue').then((data) => { const values = Object.fromEntries((data.content || []).map((item) => [item.key, item.value])); setContentValues(values); setContent({ key: 'description', value: values.description || '' }) }).catch(() => {}) }, [])
  useEffect(() => { if (Object.prototype.hasOwnProperty.call(contentValues, content.key)) setContent((old) => ({ ...old, value: contentValues[old.key] || '' })) }, [content.key, contentValues])
...
function VenueInfo({ venue }) {
  return <div className="venue-info"><div><b>会员制度</b><span>{venue.membershipInstructions || '月卡或次数卡，线下开通'}</span></div><div><b>营业规则</b><span>{venue.openingRules || '每日 08:00 — 23:00，预约按半小时计算'}</span></div><div><b>联系场馆</b><span>{venue.phone || '400-000-0000'}</span></div>{venue.announcement && <div className="venue-announcement"><b>场馆公告</b><span>{venue.announcement}</span></div>}</div>
}
