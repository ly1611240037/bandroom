...
function OwnerDashboard({ rooms, equipment }) {
  const [bookings, setBookings] = useState([]); const [plans, setPlans] = useState([]); const [issues, setIssues] = useState([]); const [error, setError] = useState('')
  const refresh = () => Promise.all([api('/api/owner/bookings'), api('/api/owner/membership/plans'), api('/api/owner/issues')]).then(([bookingData, planData, issueData]) => { setBookings(bookingData.bookings || []); setPlans(planData.plans || []); setIssues(issueData.issues || []) }).catch((err) => setError(err.message))
  useEffect(() => { refresh() }, [])
