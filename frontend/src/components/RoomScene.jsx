...
export default function RoomScene({ room, onBook, compact = false }) {
  ...
  const details = <>
    ...
    <ul className="room-equipment">{equipment.map((item, index) => <li key={item.id}><span><b>{index + 1}. {item.name} × {item.quantity}</b><small>{item.description}</small></span><Status>{item.status}</Status></li>)}</ul>
  </>;
