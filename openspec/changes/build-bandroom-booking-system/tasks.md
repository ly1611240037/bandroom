...
- [ ] 1.1 Create the React frontend and Go backend directory structure, and verify both development entry points start successfully.
- [ ] 1.2 Configure environment variables for SQLite, sessions, mail delivery, and application URLs, and verify the app starts with a documented example configuration.
- [ ] 1.3 Add shared error response, request validation, time-zone, and half-hour time utilities, and verify unit tests cover valid and invalid inputs.
...
- [ ] 3.1 Implement customer registration, password hashing, email verification, login, logout, and session handling; verify unverified customers cannot create bookings.
- [ ] 3.2 Implement password-reset request and one-time expiring reset links; verify a used or expired link cannot reset a password again.
- [ ] 3.3 Implement owner authentication and role checks for all backend management operations; verify a customer cannot access owner endpoints or another customer's data.
...
- [ ] 4.1 Implement owner management of monthly and count-based plans, including enable/disable and offline payment records; verify plan changes appear in the owner backend.
- [ ] 4.2 Implement manual card activation, 30-day monthly validity, sequential future monthly cards, and unlimited count-card validity; verify date and remaining-count calculations.
- [ ] 4.3 Implement monthly-card priority, count deduction, cancellation restoration, and eligibility checks; verify unit and integration tests cover both card types and exhausted counts.
...
- [ ] 5.1 Implement room CRUD, photos, fixed equipment, capacity display, and room status; verify maintenance and suspended rooms cannot be selected for new bookings.
- [ ] 5.2 Implement weekly schedules, special closed dates, partial-day closures, and configurable cleaning buffer with defaults of 08:00–23:00 and 30 minutes; verify affected slots are unavailable.
- [ ] 5.3 Implement public equipment CRUD, quantities, statuses, and availability calculation; verify unavailable equipment cannot be selected.
...
- [ ] 8.1 Build the owner dashboard and date-based all-room booking calendar; verify booking details show customer, room, time, equipment, membership usage, notes, and status.
